...
///
/// Hardware implementations may wish to have a single implementation support
/// different parameterizations of clic extensions. This extension defines that
/// programmability.
...
impl SMCLICCONFIG {
    ...
    /// Check how many upper bits in `clicintctl[i]` are assigned to encode the
    /// interrupt level at that privilege level.
    #[inline]
    ...
    /// Set how many upper bits in `clicintctl[i]` are assigned to encode the
    /// interrupt level at that privilege level.
    #[inline]
    ...
    /// Check how many bits are physically implemented in `clicintattr[i]`.mode

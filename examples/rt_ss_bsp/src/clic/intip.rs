...
/// > interrupt input, as
/// > determined by the clicintattr[i] field. Software writes to clicintip[i]
/// > can set or clear
...
impl INTIP {
    ...
    /// * Software cannot rely on the underlying clicintip[i] register bits used
    ///   in edge-triggered mode to hold state while in level-sensitive mode.
    #[inline]
    pub unsafe fn is_pending(self) -> bool {
        ...
        // > clicintip[i]!=0
        // > indicates an interrupt is pending to accommodate possible future expansion
        // > of the
        // > clicintip field.

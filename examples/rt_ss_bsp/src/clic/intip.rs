...
/// CLIC interrupt pending register.
///
/// > When the input is configured for edge-sensitive input, `clicintip[i]` is a
/// > read-write register
/// > that can be updated both by hardware interrupt inputs and by software. The
/// > bit is set by
/// > hardware after an edge of the appropriate polarity is observed on the
/// > interrupt input, as
/// > determined by the `clicintattr[i]` field. Software writes to
/// > `clicintip[i]`
/// > can set or clear
/// > edge-triggered pending bits directly by writes to the memory-mapped
/// > register. Edge-triggered
/// > pending bits can also be cleared when a CSR instruction that accesses
/// > xnxti includes a write.
...
impl INTIP {
    ...
    /// # Safety
    ///
    /// * The value in the `clicintip[i]` is undefined when switching from
    ///   level-sensitive mode to edge-triggered mode in `clicintattr[i]`.
    /// * Software cannot rely on the underlying `clicintip[i]` register bits
    ///   used in edge-triggered mode to hold state while in level-sensitive
    ///   mode.
    #[inline]
    pub unsafe fn is_pending(self) -> bool {
        ...
        // > Software should assume `clicintip[i] = 0` means no interrupt
        // > pending, and `clicintip[i] != 0` indicates an interrupt is pending
        // > to accommodate possible future expansion of the clicintip field.
